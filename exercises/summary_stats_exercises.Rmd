...
```{r}
# create a Table 1 for the laryngoscope data

```

Hints:

- use `select` to pick just the variables you want to include in the table
- try `str(laryngoscope)` to see which variables are being treated as numeric and which as factors, and then use `mutate` to create or modify variables as needed
- use `tbl_summary` to generate a table of descriptive statistics

There are lots of examples of things to try here https://www.danieldsjoberg.com/gtsummary/articles/tbl_summary.html 
