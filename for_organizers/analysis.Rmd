...
`r sum(r102$select_workshops___mar == 1)` signups. 

`r sum(r102$missing_values_in_r_follow_up_complete == 2)` responses on post-workshop survey.
...
```{r missing_values_ttest}
#| eval = eval_mar
mar_test <- t.test(r102$missing_values_2, r102$missing_values, paired = TRUE)
```

We saw significant improvement from pre to post after the Missing Values in R session, with respondents reporting higher ability afterward, `r print_t(mar_test, eval=eval_mar)`.

## Summary Stats

`r sum(r102$select_workshops___apr == 1)` signups. 

`r sum(r102$summary_stats_in_r_follow_up_complete == 2)` responses on post-workshop survey.
...
```{r summary_stats_ttest}
#| eval = eval_apr
apr_test <- t.test(r102$create_table_2, r102$create_table, paired = TRUE)
```

We saw significant improvement from pre to post after the Summary Statistics in R session, with respondents reporting higher ability afterward, `r print_t(apr_test, eval=eval_apr)`.

## Tidyr

`r sum(r102$select_workshops___may == 1)` signups. 

`r sum(r102$reshaping_data_in_tidyr_follow_up_complete == 2)` responses on post-workshop survey.
...
We saw significant improvement from pre to post after the Reshaping Data with tidyr session, with respondents reporting higher ability afterward, `r print_t(may_test, eval=eval_may)`.

## Data types and viz

`r sum(r102$select_workshops___jun == 1)` signups. 

`r sum(r102$data_types_and_viz_follow_up_complete == 2)` responses on post-workshop survey.
...
We saw significant improvement from pre to post after the Data Types and Visualizations in R session, with respondents reporting higher ability afterward, in both data types `r print_t(data_test, eval=eval_jun)` and visualizations `r print_t(ggplot_test, eval=eval_jun)`.
