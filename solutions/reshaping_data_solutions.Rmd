...
```{r}
pivot_longer(relig_income, cols = !religion, names_to = "income_range", values_to = "count")
```
...
### Some alternative ways to choose the columns:
```{r}
# Specify the first column then add a colon before specifying the last column
pivot_longer(relig_income, cols = `<$10k`:`Don't know/refused`, names_to = "income_range", values_to = "count")

# Specify the column numbers (this is less preferred, because if you end up moving or filtering columns, the numbers might not line up anymore)
pivot_longer(relig_income, cols = 2:11, names_to = "income_range", values_to = "count")
...
# First you need to use pivot_longer to make the years columns into rows -- these are separate observations of the same variables, so they should be in rows
world_bank_pop_2 <- world_bank_pop |>
  pivot_longer(`2000`:`2017`, names_to = "year", values_to = "variable_value")

# Second you need to use pivot_wider to separate the variables indicated in the indicator column into their own columns -- these are distinct variables that should be spread into columns for each observation (here, each country x year)
world_bank_tidy <- world_bank_pop_2 |>
  pivot_wider(names_from = indicator, values_from = variable_value)
