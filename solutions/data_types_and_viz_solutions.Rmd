---
title: "Data Types and Visualizations: Solutions"
...
```{r}
# Subject ID
str(covid_testing$subject_id)

# Age
str(covid_testing$age)

# Gender
str(covid_testing$gender)
...
ggplot(data = covid_testing, # dataset to use for plot
                   mapping = aes( # list of aesthetic mappings to use for plot
                             x = pan_day, # x-axis variable
                             y = rec_ver_tat, # y-axis variable
                             color = as.factor(demo_group))) + 
  geom_point() + 
  scale_color_manual(values = c("client" = 'red', # sets colors for each level
                                "misc adult" = 'orange',
                                "other adult" = 'green',
                                "patient" = 'blue',
                                "unidentified" = "purple")) + 
  ggtitle("Time Elapsed by Pandemic Day") + # adds title to plot
  xlab("Pandemic Day") + # changes x-axis label
  ylab("Time Elapsed between Receive and Verification Time") + # changes y-axis label 
  labs(color = "Demographic Group") +  # changes legend label
  theme_bw() + # changes plot theme to black and white
  theme(plot.title = element_text(hjust = 0.5), # centers plot title
        legend.position="left") # moves legend to bottom 
```
...
ggplot(data = covid_testing, # dataset to use for plot
                   mapping = aes( # list of aesthetic mappings to use for plot
                             x = result, # x-axis variable
                             y = age, # y-axis variable
                             color = gender) # note: using "color" for line graph
       ) + 
  geom_boxplot() + #
  scale_color_manual(values = c("male" = 'blue', # sets colors for each level
                                "female" = 'red')) + 
  ggtitle("Testing Results by Patient Age") + # adds title to plot
  xlab("Testing Result") + # changes x-axis label
  ylab("Age of Patients") + # changes y-axis label 
  labs(color = "Patient Gender") + # changes legend label (note this says "fill")
  theme_bw() + # changes plot theme to black and white
  theme(plot.title = element_text(hjust = 0.5), # centers plot title
        legend.position="top") + # moves legend to top
  facet_wrap(~payor_group)

ggsave(filename = "Practice_Figure.png", 
       plot = last_plot(),
       path = NULL)
```
